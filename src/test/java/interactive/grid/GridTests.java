...
public class GridTests {
    ...
    void testWrapAndRemoveAtEnd(FxRobot robot) {
        StackPane pane = setupStage();
        Grid grid = new Grid(items(1000, 6));
        robot.interact(() -> {
        ...
        /*
         * This is to test a nasty bug affecting all VFXContainers I believe
         * (Tested with a Grid because I discovered it while using one)
         *
         * Because of how intricate the listeners/bindings system is between the VFXContainer,
         * VFXScrollPane and VFXScrollBars, when the scroll bounds changed for whatever reason
         * (changing the list is one of them, and it's problematic) the scroll values would adjust
         * accordingly. This caused the trigger of the onPositionChanged(...) method in the managers and
         * consequently a new invalid state would be computed. In this case, the correct state must be
         * generated by the onItemsChanged(...) algorithm.
         *
         * I don't know if I properly fixed the issue, I just put some strong Flex Tape on the hole and, as
         * the test also shows, it works.
         *
         * See VFXScrollBar for more info.
         */

        // Test init
        assertState(grid, IntegerRange.of(0, 5), IntegerRange.of(0));
        ...
        // Remove an item
        robot.interact(() -> removeAll(grid, 4));
        assertState(grid, IntegerRange.of(0, 4), IntegerRange.of(0));
        assertCounter(0, 1, 1, 0, 0, 1, 0);
    }
