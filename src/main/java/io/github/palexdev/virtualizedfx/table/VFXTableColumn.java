...
///
/// 1) The [#tableProperty()]. Every column should specify the table's instance they belong to.
/// By default, as handled by [VFXTableManager#onColumnsChanged(ListChangeListener.Change)], the system will manage
/// this property automatically, by setting or re-setting (to `null`) the instance as columns are added/removed to/from the
/// table. This is an improvement over the previous implementation which required the user to pass the table's instance to
/// the constructors.
///
/// 2) The [#indexProperty()]. I believe it may be useful for every column to specify their position in
...
/// piece of sub-data for a class of type `T`, it should tell the built cells how to map to that piece of sub-data.
/// In fact, if you check the default cell implementations (e.g. [VFXSimpleTableCell]) you can see that they need
...
public abstract class VFXTableColumn<T, C extends VFXTableCell<T>> extends MFXLabeled implements WithCellFactory<T, C> {
    ...
    ///
    /// As probably already explained in [VFXTable], since columns are concrete nodes, there's the potential risk
    /// of adding duplicates in the list which would lead to a JavaFX exception. This is the exact reason why
    /// [Collections#swap(List, int, int)] cannot be used. What this method does internally, is to copy the columns
    /// to a temporary array, swap the columns in that, and finally use [ObservableList#setAll(Object\[\])].
    ...
    /// Delegates to [#swapColumns(ObservableList, int, int)] by using [VFXTable#getColumns()].
    ...
    /// @return the default function used to build cells. Uses [VFXSimpleTableCell].
    ...
    /// {@inheritDoc}
    ///
    /// Overridden to make it public. Since we use the [#prefWidthProperty()] to store the width we'd like the
    /// column to have, and for how JavaFX works, invoking [#prefWidth(double)] would return a cached value which
    ...
    /// The value will be `null` if the column is not yet in the scene graph or if the column is not part of any table.
