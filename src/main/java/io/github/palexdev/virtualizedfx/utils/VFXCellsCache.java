...
/// Simple cache implementation for virtualized containers that produce cells of type [VFXCell].
...
///
/// **Note** however that to generate cells, the cache must know the function to produce them. The function must be
...
///
/// - The mapping [Item->VFXCell] raises the problem of updating the cache when the items list changes.
...
public class VFXCellsCache<T, C extends VFXCell<T>> {
    ...
    /// available and just in need of an update ([VFXCell#updateIndex(int)], [VFXCell#updateItem(Object)]).
    ///
    /// @throws NullPointerException if [#getCellFactory()] returns `null`
    ...
    /// Beware this can return a `null` value, if it's not, [VFXCell#onDeCache()] is automatically invoked.
    public C take() {
        C c = queue.poll();
    ...
    /// Wraps the result of [#take()] in an [Optional] instance.
