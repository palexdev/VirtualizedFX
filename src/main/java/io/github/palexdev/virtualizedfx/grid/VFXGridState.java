...
/// Immutable object to represent the state of a [VFXGrid] in a specific moment in time. In other words, each and
...
/// When implementing the [VFXGrid]'s "infrastructure", the state was one of the first classes to be written since
...
/// mean that the specialized data structure [StateMap] could not be used. We would need to adapt it to use our
...
///
/// The important thing to notice here is that essentially what the grid implicitly does is **converting a position** from
/// the list **to a pair of coordinates**. Well, we can also do the opposite, starting from the two ranges, we can get
/// the corresponding position in the list for each pair of coordinates.
///
/// **Linear Index**: A linear index refers to a single integer value used to access an element in a one-dimensional
...
///
/// **Subscripts**: Subscripts are used to refer to the indices of individual dimensions in a multidimensional array.
...
///
/// 1) the cells are still stored in a [StateMap], but the index used in the mapping is the **linear index**.
/// Which means that if you want to get the coordinates, you'll have to convert them back using [GridUtils#indToSub(int, int)].
/// 2) this is the first time in my Java developer career I've used labels to break out of loops.
/// You see when you use two nested for loops to iterate over the rows and columns ranges,
/// it may happen that the resulting linear index is non-existing in the list.
/// In other words, the linear index is greater than `itemsNum - 1`. In such cases, it's useless to
...
public class VFXGridState<T, C extends VFXCell<T>> {
    ...
    // Static Properties
    //================================================================================
    /// Special instance of `VFXGridState` used to indicate that no cells can be present in the viewport at
    ...
    /// Delegates to [#addCell(int, Object, VFXCell)].
    protected void addCell(int index, C cell) {
        addCell(index, grid.getItems().get(index), cell);
    }

    /// Adds the given cell to the [StateMap] of this state object.
    ...
    /// Removes a cell from the [StateMap] for the given linear index. If the cell is not found, the next attempt
    /// is to remove it by the item at the given linear index in the [VFXGrid#itemsProperty()].
    ...
    /// Removes a cell from the [StateMap] for the given item.
    ...
    /// @return the [VFXGrid] instance this state is associated to
    ...
    /// @return the number of cells in the [StateMap]
    public int size() {
        return cells.size();
    }
