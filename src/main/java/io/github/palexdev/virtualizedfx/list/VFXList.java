...
///
/// - The default behavior implementation, [VFXListManager], can be considered as the name suggests more like
...
/// algorithms, the [VFXListManager] adopts a much simpler strategy while still trying to keep the cell updates count
/// as low as possible to improve performance. See [VFXListManager#onItemsChanged()].
///
/// - The function used to generate the cells, called "cellFactory", can be changed anytime, even at runtime,
/// see [VFXListManager#onCellFactoryChanged()].
///
/// - The component is around the concept of a fixed cell size for all cells, this parameter can be controlled
/// through the [#cellSizeProperty()], and can also be changed anytime, see [VFXListManager#onCellSizeChanged()].
///
/// - Similar to the `VBox` pane, this container allows you to evenly space the cells in the viewport by setting the
/// [#spacingProperty()]. See [VFXListManager#onSpacingChanged()].
///
...
/// [VFXListHelper]. You are allowed to change the helper through the [#helperFactoryProperty()].
///
...
/// Instead, it's much more likely that the [VFXCell#updateItem(Object)] will be simple and faster.
...
public class VFXList<T, C extends VFXCell<T>> extends MFXControl
    ...
    /// @return the default function used to produce a [VFXListHelper] according to the container's orientation.
    ...
    /// Delegate for [VFXListState#getRange()]
    ...
    /// {@inheritDoc}
    ///
    /// Also, the default implementation (see [VFXListHelper.VerticalHelper] or [VFXListHelper.HorizontalHelper]), adds
    /// double the number specified by the enum constant, because these buffer cells are added both at the top and at the
    /// bottom of the container. The default value is [BufferSize#MEDIUM].
    ///
    ...
    /// direction of the current [#orientationProperty()].
    ///
    ...
    ///
    /// **Side note:** the clip is a [Rectangle], now for some fucking reason,
    ...
    /// {@inheritDoc}
    ///
    /// In case the orientation is set to [Orientation#VERTICAL], this is to be considered a 'virtual' position,
    /// as the container will never reach unreasonably high values for performance reasons.
    /// See [VFXListHelper.VerticalHelper] to understand how virtual scroll is handled.
    @Override
    ...
    /// {@inheritDoc}
    ///
    /// In case the orientation is set to [Orientation#HORIZONTAL], this is to be considered a 'virtual' position,
    /// as the container will never reach unreasonably high values for performance reasons.
    /// See [VFXListHelper.HorizontalHelper] to understand how virtual scroll is handled.
    @Override
