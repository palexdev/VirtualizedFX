...
public interface VFXListHelper<T, C extends VFXCell<T>> extends VFXContainerHelper<T, VFXList<T, C>> {
    ...
    /// @return the number of cells visible in the viewport. Not necessarily the same as [#totalNum()]
    int visibleNum();
    ...
    /// spacing as an extension of the [VFXList#cellSizeProperty()]. In other words, for the helper to still
    /// produce valid ranges, it is enough to sum the spacing to the cell size when the latter is needed.
    /// This is a shortcut for `getContainer().getCellSize() + getContainer().getSpacing()`.
    ...
    /// (cannot be negative) and the sum between this start index and the total number of needed cells given by [#totalNum()],
    ...
    /// The horizontal position is just the current [VFXList#hPosProperty()] but negative. The vertical
    /// position is the one virtualized.
    /// First we get the range of items to display and the total cell size given by [#getTotalCellSize()], yes, the
    ...
    class VerticalHelper<T, C extends VFXCell<T>> extends AbstractHelper<T, C> {
        ...
        /// {@inheritDoc}
        ///
        /// Given by `Math.ceil(listHeight / totalCellSize)`.
        @Override
        ...
        /// {@inheritDoc}
        ///
        /// If [VFXList#fitToViewportProperty()] is true, then the computation will always return the
        /// list's width, otherwise the node width is computed by [LayoutUtils#boundWidth(Node)].
        /// Also, in the latter case, if the found width is greater than the current max x, then the property
        /// [#virtualMaxXProperty()] is updated with the new value.
        @Override
    ...
    /// Concrete implementation of [AbstractHelper] for [Orientation#HORIZONTAL]. Here the range of items to
    /// display and the viewport position are defined as follows:
    ///
    /// - the range is given by the [#firstVisible()] element minus the buffer size ([VFXList#bufferSizeProperty()]),
    ...
    class HorizontalHelper<T, C extends VFXCell<T>> extends AbstractHelper<T, C> {
        ...
        /// {@inheritDoc}
        ///
        /// Given by `Math.ceil(listWidth / totalCellSize)`.
        @Override
        public int visibleNum() {
            double size = getTotalCellSize();
            return size > 0 ?
        ...
        ///
        /// If [VFXList#fitToViewportProperty()] is true, then the computation will always return the
        /// list's height, otherwise the node height is computed by [LayoutUtils#boundHeight(Node)].
        /// Also, in the latter case, if the found height is greater than the current max y, then the property
        /// [#virtualMaxYProperty()] is updated with the new value.
        @Override
        public double computeSize(Node node) {
            boolean fitToViewport = container.isFitToViewport();
            if (fitToViewport) {
